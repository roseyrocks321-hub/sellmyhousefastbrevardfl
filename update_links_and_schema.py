...
CITY_PAGES = {
    ...
    "west melbourne": "/sell-my-house-fast-melbourne",
    "eau gallie": "/sell-my-house-fast-melbourne",
    "suntree": "/sell-my-house-fast-melbourne",
}

CITY_LINK_TEXTS = {
    "melbourne": "sell my house fast in Melbourne",
    "palm bay": "sell my house fast in Palm Bay",
    "titusville": "sell my house fast in Titusville",
    "cocoa": "sell my house fast in Cocoa",
    "cocoa beach": "sell my house fast in Cocoa Beach",
    "rockledge": "sell my house fast in Rockledge",
    "merritt island": "sell my house fast in Merritt Island",
    "satellite beach": "sell my house fast in Satellite Beach",
    "indialantic": "sell my house fast in Indialantic",
    "indian harbour beach": "sell my house fast in Indian Harbour Beach",
    "cape canaveral": "sell my house fast in Cape Canaveral",
    "viera": "sell my house fast in Viera",
    "melbourne beach": "sell my house fast in Melbourne Beach",
    "west melbourne": "sell my house fast in West Melbourne",
    "eau gallie": "sell my house fast in Eau Gallie",
    "suntree": "sell my house fast in Suntree",
}
...
CITY_FAQS = {
    "cape-canaveral": [
        ("How fast can I sell my house in Cape Canaveral, FL?",
         "You can sell your Cape Canaveral home for cash in as little as 7 days. We provide a no-obligation cash offer within 24 hours of viewing your property and can close on your schedule—whether that's next week or next month. Our cash process bypasses traditional financing delays, inspections, and buyer contingencies."),
    ...
    "indian-harbour-beach": [
        ...
        ("How does the beachside location affect home sales in Indian Harbour Beach?",
         "Indian Harbour Beach's desirable barrier island location supports strong property values, but also means buyers often have strict inspection requirements related to coastal conditions. Cash buyers purchase as-is, so you don't need to worry about wind mitigation reports, elevation certificates, or coastal erosion concerns before selling."),
...
def add_links_to_blog(filepath):
    ...
    # Determine relevant cities based on filename and content
    lower_content = content.lower()
    cities_found = {}
    for city, url in CITY_PAGES.items():
        count = lower_content.count(city)
        if count > 0:
            cities_found[city] = count

    # Add filename-based hints
    if "melbourne" in filename.lower():
        cities_found["melbourne"] = cities_found.get("melbourne", 0) + 100
    if "palm bay" in filename.lower() or "palmbay" in filename.lower():
        cities_found["palm bay"] = cities_found.get("palm bay", 0) + 100
    if "titusville" in filename.lower():
        cities_found["titusville"] = cities_found.get("titusville", 0) + 100
    if "cocoa beach" in filename.lower() or "cocoabeach" in filename.lower():
        cities_found["cocoa beach"] = cities_found.get("cocoa beach", 0) + 100
    elif "cocoa" in filename.lower():
        cities_found["cocoa"] = cities_found.get("cocoa", 0) + 100
    if "rockledge" in filename.lower():
        cities_found["rockledge"] = cities_found.get("rockledge", 0) + 100
    if "merritt island" in filename.lower() or "merrittisland" in filename.lower():
        cities_found["merritt island"] = cities_found.get("merritt island", 0) + 100
    if "satellite beach" in filename.lower() or "satellitebeach" in filename.lower():
        cities_found["satellite beach"] = cities_found.get("satellite beach", 0) + 100
    if "indialantic" in filename.lower():
        cities_found["indialantic"] = cities_found.get("indialantic", 0) + 100
    if "indian harbour beach" in filename.lower() or "indianharbour" in filename.lower():
        cities_found["indian harbour beach"] = cities_found.get("indian harbour beach", 0) + 100
    if "cape canaveral" in filename.lower() or "capecanaveral" in filename.lower():
        cities_found["cape canaveral"] = cities_found.get("cape canaveral", 0) + 100
    if "viera" in filename.lower():
        cities_found["viera"] = cities_found.get("viera", 0) + 100
    if "melbourne beach" in filename.lower() or "melbournebeach" in filename.lower():
        cities_found["melbourne beach"] = cities_found.get("melbourne beach", 0) + 100

    # Sort by frequency
    sorted_cities = sorted(cities_found.items(), key=lambda x: x[1], reverse=True)

    # Pick top 1-2 cities, excluding generic "melbourne" if more specific exists for melbourne beach
    selected = []
    for city, count in sorted_cities:
        if city in CITY_PAGES and city in CITY_LINK_TEXTS:
            # Don't add link if it already exists
            if f'href="{CITY_PAGES[city]}"' in content:
                continue
    ...
    # Find a good paragraph to insert links (after first </h1> or </h2>, or in a paragraph)
    # Strategy: add a contextual sentence with link near the first mention of the city
    for city in selected:
        url = CITY_PAGES[city]
        link_text = CITY_LINK_TEXTS[city]

        # Try to find first standalone mention in a paragraph (not already in a link or heading)
        # Match the city name in a paragraph, not inside an <a> tag or <h>
        pattern = re.compile(rf'(<p[^>]*>.*?)({re.escape(city.title() if city != "eau gallie" else "Eau Gallie")})(.*?)(</p>)', re.IGNORECASE | re.DOTALL)
        match = pattern.search(content)
        if not match:
            # Try lowercase version
            pattern = re.compile(rf'(<p[^>]*>.*?)({re.escape(city)})(.*?)(</p>)', re.IGNORECASE | re.DOTALL)
            match = pattern.search(content)

        if match:
            start, city_text, mid, end = match.groups()
            # Only link if not already inside an <a> tag
            # Simple heuristic: check if there's an <a before this in the same paragraph without closing </a>
            paragraph_before_city = start + city_text
            if paragraph_before_city.count('<a') <= paragraph_before_city.count('</a>'):
                new_paragraph = start + f'<a href="{url}" style="color:#2c5282;text-decoration:none;font-weight:500;">{link_text}</a>' + mid + end
                content = content[:match.start()] + new_paragraph + content[match.end():]
                continue

        # Fallback: add a link box after the first </h1>
        h1_end = content.find('</h1>')
        if h1_end != -1:
            insert_pos = h1_end + 5
            link_html = f'\n<p>Looking to <a href="{url}" style="color:#2c5282;text-decoration:none;font-weight:500;">{link_text}</a>? We buy houses for cash throughout Brevard County.</p>\n'
...
def replace_faq_schema(filepath):
    ...
    # Remove existing FAQPage schema blocks
    # Pattern matches <script type="application/ld+json"> ... "@type": "FAQPage" ... </script>
    faq_pattern = re.compile(
        r'<script\s+type="application/ld+json"\s*>\s*\{[^}]*"@type"\s*:\s*"FAQPage"[^<]*\}\s*</script>',
        re.IGNORECASE | re.DOTALL
    )
    content = faq_pattern.sub('', content)

    # Also catch variants with extra whitespace or comments
    faq_pattern2 = re.compile(
        r'<!--\s*FAQ Schema\s*-->\s*<script\s+type="application/ld+json"\s*>\s*\{[^}]*"@type"\s*:\s*"FAQPage"[^<]*\}\s*</script>',
        re.IGNORECASE | re.DOTALL
    )
    content = faq_pattern2.sub('', content)
