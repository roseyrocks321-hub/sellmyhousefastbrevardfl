...
def add_blog_links(filepath):
    ...
    for city in selected:
        ...
        # that does NOT already contain a link to this URL
...
def remove_faq_schemas(content):
    ...
    for _ in range(20):
        ...
        if '"@type": "FAQPage"' in block or '"@type":"FAQPage"' in block:
            ...
            # Move past this script tag to avoid infinite loop
            content = content[:idx] + content[idx + len(script_open):]
            # But we need to keep the rest, so let's just replace the opening tag with a marker
            # Actually better: find the NEXT script tag after close_idx
            # Reconstruct by keeping before and after, but we already advanced past
            # Let's put back and skip differently
