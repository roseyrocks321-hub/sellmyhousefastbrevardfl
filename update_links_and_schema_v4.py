...
def find_city_mentions(content, filename):
    ...
    for city in CITY_PAGES:
        # Use word-boundary-like matching: not preceded/followed by letter
        pat = r'(?<![a-z])' + re.escape(city) + r'(?![a-z])'
        scores[city] = len(re.findall(pat, lower))
...
def add_blog_links(filepath):
    ...
    for city in selected:
        ...
        # Try to find first paragraph containing the city name, searching from after last insertion
        search_start = inserted_positions[-1] if inserted_positions else 0
        ...
        if match:
            ...
            if link_html not in content:
                ...
                inserted_positions.append(insert_pos + len(link_html) + 1)
                continue
...
def remove_faq_schemas(content):
    """Remove all FAQPage schema blocks using string operations."""
    ...
    for _ in range(20):
        ...
        close_idx = content.find(script_close, idx + len(script_open))
        if close_idx == -1:
            break

        block = content[idx:close_idx + len(script_close)]
        if '"@type": "FAQPage"' in block or '"@type":"FAQPage"' in block:
            before = content[:idx]
            stripped = before.rstrip()
            comment_start = stripped.rfind('<!--')
            if comment_start != -1:
                comment_part = stripped[comment_start:]
                if '-->' in comment_part and 'FAQ' in comment_part.upper():
                    before = stripped[:comment_start]
            content = before + content[close_idx + len(script_close):]
        else:
            # Skip past this script tag by replacing opening with marker
            content = content[:idx] + "<!--SKIPPED-->" + content[idx+len(script_open):]

    content = content.replace("<!--SKIPPED-->", script_open)
    content = re.sub(r'\n{3,}', '\n\n', content)
    return content
...
def replace_faq_schema(filepath):
    ...
    content = remove_faq_schemas(content)
